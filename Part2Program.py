...
for dataset in datasets:
    ...
    sizeReduction = 7       #value of 1 corresponds to reading all data. Value of 2 for example, fits the data against 1 in every 2 points (~ 50% time reduction)
    #for the above, sizeReduction = 7 seems to be about the sweet spot, and gives Chi2 ~ 1
    ...
    if dataset in ["Data00", "Data0", "Data1", "Data2", "Data3"]:
        ...
        w0_array = np.linspace(-1, 1, nSmall)    # Array of Dark Energy Equation of state, w0
        wa_array = np.linspace(0, 1, nSmall)     # Array of change of Dark energy equation of state, wa
    chi2 = np.ones((n, n, nSmall, nSmall, nSmall)) * np.inf  # Array to hold our chi2 values, set initially to super large values
    print("Number of parameter samples iterated over = ", n**2 * nSmall**3)
    ...
    ibest   = np.unravel_index(indbest, [n, n, nSmall, nSmall, nSmall]) # Converts the best fit index to the 2d version (i,j)
    ...
    ax.contour(oms, ols, contourMatrix, cmap="copper", **{'levels':[2.30,6.18,11.83]})
    ...
    text.write('\nThe total number of models compared are ' + str(n**2 * nSmall**3))
