...
def lnlike0(params, zs=None, mu=None, muerr=None):
    """This function serves as the log-likelihood function for the MCMC implementation. """
    om, ol = params
    LnLike = -0.5 * (np.sum((model(params, zs, mu, muerr) - mu)**2 / muerr**2) + ((om - 0.3) / 0.05)**2)
    if np.isnan(LnLike):
        return 0
    return LnLike

def lnlike1(params, zs=None, mu=None, muerr=None):
    """This function serves as the log-likelihood function for the MCMC implementation. """
    om, ol = params
    LnLike = -0.5 * (np.sum((model(params, zs, mu, muerr) - mu)**2 / muerr**2) + ((om + ol - 1) / 0.05)**2)
    if np.isnan(LnLike):
        return 0
    return LnLike
...
def lnprob0(params, zs=None, mu=None, muerr=None):
    ...
    lp = lnprior0(params)        #checks if the parameters are within the prior parameter space
    if (not np.isfinite(lp)) or np.isnan(lp):       #dismisses this model if the parameters don't fit the allowed space.
        return -np.inf
    return lp + lnlike0(params, zs, mu, muerr)

def lnprob1(params, zs=None, mu=None, muerr=None):
    """This is the log probability function for the MCMC implementation. It effectively gives the relative probability of some model
    giving the correct fit.
    If the parameters aren't within the allowed parameter space, emcee requires that this function returns -np.inf"""
    ...
    if (not np.isfinite(lp)) or np.isnan(lp):       #dismisses this model if the parameters don't fit the allowed space.
        return -np.inf
    return lp + lnlike1(params, zs, mu, muerr)
...
def main():
    ...
    for i in range(0, 2):
        if i == 0:
            ...
            sampler, pos, prob, state = mcmc(p0, nwalkers, niter, ndim, lnprob0, data)   #initiate the mcmc
            ...
            plt.savefig(f'Part Two Graphs/{dataset} Priors/{dataset} OM=0.3 MCMC Corner Plot.png', dpi=200, bbox_inches='tight', pad_inches = 0.01, transparent=False)
            plt.savefig(f'Part Two Graphs/{dataset} Priors/{dataset} OM=0.3 MCMC Corner Plot.pdf', dpi=200, bbox_inches='tight', pad_inches = 0.01, transparent=False)
            plt.close(fig)
            
            fig, ax = plt.subplots()
            ax.hist(flat_samples[:, 1], bins=bins, histtype='step', color='#307CC0')
            ax.set_xlim(0, 1); ax.axvline(mean_params[1], color='r', linestyle='-')
            ax.tick_params(axis='y', which='both', left=False, right=False, top=False, bottom=False, labelbottom=False, labelleft=False)
            plt.savefig(f'Part Two Graphs/{dataset} Priors/{dataset} OM=0.3 MCMC Histogram.png', dpi=200, bbox_inches='tight', pad_inches = 0.01, transparent=False)
            plt.savefig(f'Part Two Graphs/{dataset} Priors/{dataset} OM=0.3 MCMC Histogram.pdf', dpi=200, bbox_inches='tight', pad_inches = 0.01, transparent=False)
            plt.close(fig)
            ...
            fig.savefig(f'Part Two Graphs/{dataset} Priors/{dataset} OM=0.3 MCMC Norm Mag vs Redshift.png', dpi=200, bbox_inches='tight', pad_inches = 0.01, transparent=False)
            fig.savefig(f'Part Two Graphs/{dataset} Priors/{dataset} OM=0.3 MCMC Norm Mag vs Redshift.pdf', dpi=200, bbox_inches='tight', pad_inches = 0.01, transparent=False)
            plt.close(fig)
            ...
            ax.legend(frameon=False)
            fig.savefig(f'Part Two Graphs/{dataset} Priors/{dataset} OM=0.3 MCMC Magnitude vs Redshift.png', dpi=200, bbox_inches='tight', pad_inches = 0.01, transparent=False)
            fig.savefig(f'Part Two Graphs/{dataset} Priors/{dataset} OM=0.3 MCMC Magnitude vs Redshift.pdf', dpi=200, bbox_inches='tight', pad_inches = 0.01, transparent=False)
            plt.close(fig)
            ...
            text = open(f'Part Two Graphs/{dataset} Priors/{dataset} OM=0.3 MCMC details.txt', "w")
            ...
            sampler, pos, prob, state = mcmc(p0, nwalkers, niter, ndim, lnprob1, data)   #initiate the mcmc
            samples = sampler.flatchain     #this obtains the final data
            extra_params = [0, -1, 0]      #since these are the simple datasets, we need to append the orad, w0, and wa values onto the end of our parameter array
            ...
            mean_params = np.append(mean_params, extra_params)      #this fixes the first few dataset parameters that are missing orad, w0, wa
            plt.savefig(f'Part Two Graphs/{dataset} Priors/{dataset} OMOL=1 MCMC Corner Plot.png', dpi=200, bbox_inches='tight', pad_inches = 0.01, transparent=False)
            plt.savefig(f'Part Two Graphs/{dataset} Priors/{dataset} OMOL=1 MCMC Corner Plot.pdf', dpi=200, bbox_inches='tight', pad_inches = 0.01, transparent=False)
            plt.close(fig)
            ...
            ax.set_xlabel('Redshift ($z$)'); ax.set_ylabel('Apparent Magnitude [Normalised to (0,0)]')
            ax.legend(frameon=False)
            fig.savefig(f'Part Two Graphs/{dataset} Priors/{dataset} OMOL=1 MCMC Norm Mag vs Redshift.png', dpi=200, bbox_inches='tight', pad_inches = 0.01, transparent=False)
            fig.savefig(f'Part Two Graphs/{dataset} Priors/{dataset} OMOL=1 MCMC Norm Mag vs Redshift.pdf', dpi=200, bbox_inches='tight', pad_inches = 0.01, transparent=False)
            plt.close(fig)
            ...
            ax.set_xlabel('Redshift ($z$)'); ax.set_ylabel('Apparent Magnitude')
            ax.legend(frameon=False)
            fig.savefig(f'Part Two Graphs/{dataset} Priors/{dataset} OMOL=1 MCMC Magnitude vs Redshift.png', dpi=200, bbox_inches='tight', pad_inches = 0.01, transparent=False)
            fig.savefig(f'Part Two Graphs/{dataset} Priors/{dataset} OMOL=1 MCMC Magnitude vs Redshift.pdf', dpi=200, bbox_inches='tight', pad_inches = 0.01, transparent=False)
            plt.close(fig)
        
        
        
            #the following text writes to a .txt file with all of the relevant parameters of the best fit model so that we don't need to run the code again to see :)
            text = open(f'Part Two Graphs/{dataset} Priors/{dataset} OMOL=1 MCMC details.txt', "w")
            text.write(str('The program ran with ' + str(nwalkers) + ' walkers over ' + str(niter) + ' iterations.'))
            text.write('\nThis corresponds to effectively ' + str(nwalkers * niter) + ' models sampled.')
            text.write('\nThe mean value of each parameter is (om, ol, orad, w0, wa)=(%.3f, %.3f, %.3f, %.3f, %.3f)'%(mean_params[0], mean_params[1], mean_params[2], mean_params[3], mean_params[4]))
            text.write('\nThe maximum likelihood value of each parameter is (om, ol, orad, w0, wa)=(%.3f, %.3f, %.3f, %.3f, %.3f)'%(maxlikeli[0], maxlikeli[1], maxlikeli[2], maxlikeli[3], maxlikeli[4]))
