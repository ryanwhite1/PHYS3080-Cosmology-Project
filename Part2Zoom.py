...
for dataset in datasets:
    ...
    for h in range(0, runs):
        ...
        if i > 0:
            n = 5                  # Increase this for a finer grid.
            nSmall = 5
            nTiny = 4
            olsUnc = ols[1] - ols[0]; omsUnc = oms[1] - oms[0]; oradsUnc = orads[1] - orads[0]; w0Unc = w0_array[1] - w0_array[0]; waUnc = wa_array[1] - wa_array[0]
            ols = np.linspace(ols[ibest[1]] - 2*olsUnc, ols[ibest[1]] + 2*olsUnc, n)   # Array of cosmological constant values
            ...
            if dataset in ["Data00", "Data0", "Data1", "Data2", "Data3"]:
                #the above datasets are computed according to simple models, and so the compute time can be reduced by reducing the sample space
                oms = np.linspace(0.5 * oms[ibest[0]], 1.2 * oms[ibest[0]], n)   # Array of matter densities
                orads = [0]     #data sets have orad = 0
                w0_array = [-1]     #data sets have w0 = -1
                wa_array = [0]      #data sets have wa = 0
            else:        
                oms = np.linspace(oms[ibest[0]] - 2*omsUnc, oms[ibest[0]] + 2*omsUnc, n)   # Array of matter densities
                orads = np.linspace(orads[ibest[2]] - 2*oradsUnc, orads[ibest[2]] + 2*oradsUnc, nSmall)    # Array of radiation density values
                w0_array = np.linspace(w0_array[ibest[3]] - 2*w0Unc, w0_array[ibest[3]] + 2*w0Unc, nSmall)    # Array of Dark Energy Equation of state, w0
                wa_array = np.linspace(wa_array[ibest[4]] - 2*waUnc, wa_array[ibest[4]] + 2*waUnc, nSmall)     # Array of change of Dark energy equation of state, wa
            ...
            if dataset in ["Data00", "Data0", "Data1", "Data2", "Data3"]:
                ...
                w0_array = np.linspace(-2, 0, nSmall)    # Array of Dark Energy Equation of state, w0
                wa_array = np.linspace(-2, 2, nSmall)     # Array of change of Dark energy equation of state, wa
                ...
                for k, orad in enumerate(orads):                           
                    for l, w0 in enumerate(w0_array):
                        for m, wa in enumerate(wa_array):
                            mu_model = dist_mod(zsReduced, om, ol, orad, w0, wa)     # calculate the distance modulus vs redshift for that model 
